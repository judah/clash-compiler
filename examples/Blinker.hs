...
topEntity
  :: Clock "Input" Regular
  -- ^ Incoming clock, straight from the clock crystal.
  -> Signal "Input" Bool
...
topEntity clk20 rstBtn modeBtn =
 ...
 where
  ...
  -- not pressed. We want it exactly the other way around though!
  rst = unsafeToActiveHighReset (not <$> rstBtn)
  ...
  -- remain active while the PLL is NOT stable, hence the inversion with 'not'.
  rstSync =
    resetSynchronizer
      clk50
      (unsafeToActiveHighReset (not <$> pllStable))
