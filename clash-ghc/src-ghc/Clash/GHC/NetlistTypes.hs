...
import Clash.Signal.Internal
  (ClockKind (..), ResetPolarity(..), ActiveEdge(..), ResetKind(..)
  ,InitBehavior(..))
    ...
    go reprs m ty@(tyView -> TyConApp tc args) = runExceptT $
      case nameOcc tc of
        ...
        "Clash.Signal.Internal.KnownDomain"
          | [_tag, conf] <- args
          -> do
            let TyConApp _ [tag0, period0, edge0, rstKind0, init0] = tyView conf
            ...
            returnN (KnownDomain (pack tag1) period1 edge1 rstKind1 init1)

        "Clash.Signal.Internal.Clock"
          | [tag0, clkKind0] <- args
          -> do
            tag1 <- domTag tag0
            clkKind1 <- clockKind m clkKind0
            returnN (Clock (pack tag1) clkKind1)

        "Clash.Signal.Internal.Reset"
          | [tag0, rstPolarity0] <- args
          -> do
            tag1 <- domTag tag0
            rstPolarity1 <- resetPolarity m rstPolarity0
            returnN (Reset (pack tag1) rstPolarity1)
...
clockKind =
  fromType
    (showName ''ClockKind)
    [ (showName 'Regular, Regular)
    , (showName 'Enabled, Enabled) ]
