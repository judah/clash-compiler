...
import           Clash.Core.Type             (TypeView (..), Type, applyFunTy,
                                              isPolyFunCoreTy,
                                              normalizeType,
                                              splitFunTy, typeKind,
                                              tyView, undefinedTy)
...
recToLetRec (TransformContext is0 []) e = do
  ...
  where
    ...
    eqArg tcm v1 v2@(collectArgs -> (Data _, args'))
      ...
      = if isClassConstraint (termType tcm v1) then
    ...
    -- Recursively check if data construction is equal to some variable.
    --
