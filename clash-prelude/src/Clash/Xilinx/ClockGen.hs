...
-- @
clockWizard
  :: forall tagIn tagOut periodIn periodOut edge init polarity name
   . ( KnownDomain tagIn ('Domain tagIn periodIn edge 'Asynchronous init polarity)
     , KnownDomain tagOut ('Domain tagOut periodOut edge 'Asynchronous init polarity) )
  => SSymbol name
  ...
  -> Reset tagIn
  -- ^ Reset for the PLL
  -> (Clock tagOut, Enable tagOut)
  -- ^ (Stable PLL clock, PLL lock)
clockWizard _ clk rst =
  (unsafeCoerce clk, unsafeCoerce rst)
...
clockWizardDifferential
  ...
  -> Reset tagIn
  -- ^ Reset for the PLL
  -> (Clock tagOut, Signal tagOut Bool)
  -- ^ (Stable PLL clock, PLL lock)
clockWizardDifferential _name (Clock _) (Clock _) rst =
  (Clock SSymbol, unsafeCoerce rst)
